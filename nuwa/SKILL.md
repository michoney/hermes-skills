...
### Hermes 运行环境适配（实测 2026-08，蒸馏 Michael Sitarzewski 验证）

在 Hermes 环境跑女娲流程时的三个实测坑与解法：

1. **并行 Agent 上限**：Hermes delegate_task 最多 3 个并发子代理。6 个维度合并为 **3 个 Agent × 2 维度**（如：著作+时间线 / 对话+表达 / 他者+决策），每个 Agent 内串行完成两个维度。
2. **子代理工具调用上限导致调研文件写不完**：实测 3/3 子代理都在抓取阶段耗尽 50 次工具调用上限，`references/research/*.md` 全部未落盘，内容只存在于返回总结里。对策：① spawn 时明确要求「先写文件再深挖」，把写文件放在调用序列最前面；② 父代理必须做好从子代理总结中提取内容自行落盘的准备——总结里通常有完整素材。
3. **quality_check.py 崩溃**：本机 python3 跑 `tuple[bool, str]` 注解报 `TypeError: 'type' object is not subscriptable`。替代：内联质量检查（心智模型 3-7 个 / 每个模型有「局限」标注 / 有表达DNA段落 / 诚实边界 ≥4 条 / 内在张力 ≥2 对 / 一手来源为主 / description 约 300 字内）。

另外：调研文件路径必须给子代理**绝对路径**；子代理共享浏览器会话会被其他会话劫持导航，属正常现象，让子代理失败重试即可。
